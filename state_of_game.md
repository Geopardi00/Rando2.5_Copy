...
- Grounded movement now uses the 14-frame looping `stealth_walk` animation while hidden. Current Player-scene tuning limits it to `25%` of normal horizontal speed; air control retains normal speed, and leaving stealth restores the regular walk animation and movement speed.
- Hidden-player feedback uses a cool dark modulation on the existing animated sprite plus a synchronized outline-only `AnimatedSprite2D` overlay. The outline gently cycles between two blue-white colors, inherits invulnerability blinking, and leaves the underwater material untouched. `GameUI` shows a contextual bottom-center `Q / RB` prompt.
- While stealth is active, enemy actors temporarily render one layer above the player and return to their exact authored `z_index`/relative-Z settings when stealth ends. Legacy hazards that happen to use the `enemy` group are excluded from this visual override.
